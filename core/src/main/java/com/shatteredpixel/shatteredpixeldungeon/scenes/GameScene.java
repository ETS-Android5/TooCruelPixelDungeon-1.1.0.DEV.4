...
public class GameScene extends PixelScene {
	...
	public void create() {
		...
		if (Dungeon.depth().chapter() == Chapter.SEWERS) {
			...
			Music.INSTANCE.playTracks(
					new String[]{Assets.Music.HALLS_1, Assets.Music.HALLS_2, Assets.Music.HALLS_2, Assets.Music.PYROJOKE},
					new float[]{1, 1, 0.5f, pyroChance},
					false);
		}
		...
		if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
			...
			if (Dungeon.hero.hasTalent(Talent.ROGUES_FORESIGHT)
				...
				Random.pushGenerator(Dungeon.seedCurDepth());
					if (reqSecrets <= 0 && Random.Int(2) <= Dungeon.hero.pointsInTalent(Talent.ROGUES_FORESIGHT)){
