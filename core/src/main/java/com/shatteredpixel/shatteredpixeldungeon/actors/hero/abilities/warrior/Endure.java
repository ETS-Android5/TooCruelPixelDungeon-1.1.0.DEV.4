	...
	public static class EndureTracker extends FlavourBuff implements AttackAmplificationBuff {
		...
		public void endEnduring(){
			...
			for (Char ch : Actor.chars()){
				if (ch.alignment == Char.Alignment.ENEMY && Dungeon.level.distance(target.pos(), ch.pos()) <= 4){
					nearby ++;
				}
			}
			damageBonus *= 2f + (nearby*0.1f*Dungeon.hero.pointsInTalent(Talent.EVEN_THE_ODDS));

			hitsLeft = 2+2*Dungeon.hero.pointsInTalent(Talent.SUSTAINED_RETRIBUTION);
