	...
	public static void main (String[] args) {
				...
				if (exceptionMsg.contains("Couldn't create window")){
					...
					TinyFileDialogs.tinyfd_messageBox(title + " Has Crashed!",
							title + " has run into an error it can't recover from and has crashed, sorry about that!\n\n" +
									"If you could, please report this error message to the developer (juh9870#8970 on discord):\n\n" +
		...
		if (SharedLibraryLoader.isWindows) {
			if (System.getProperties().getProperty("os.name").equals("Windows XP")) {
				basePath = "Application Data/.hahhujanapi/Not Too Cruel Pixel Dungeon/";
			} else {
				basePath = "AppData/Roaming/.hahhujanapi/Not Too Cruel Pixel Dungeon/";
			}
		} else if (SharedLibraryLoader.isMac) {
			basePath = "Library/Application Support/Not Too Cruel Pixel Dungeon/";
		} else if (SharedLibraryLoader.isLinux) {
			basePath = ".shatteredpixel/not-too-cruel-pixel-dungeon/";
		}
